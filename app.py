...
if st.session_state.resume_text:
    ...
    feedback_input = st.sidebar.text_area("Ask for feedback:", height=90, key="feedback_input")

    # Button to send request
    if st.sidebar.button("Send feedback request", key="feedback_button"):
        feedback_output = ""  # placeholder for AI response

        # Only wrap the API call in spinner
        with st.spinner("Getting feedback..."):
            try:
                resume_text = st.session_state.enhanced_text or st.session_state.resume_text
                prompt = f"Provide concise actionable feedback on this resume:\n\n{resume_text}"

                if use_gemini and GEMINI_KEY:
                    model = genai.GenerativeModel("models/gemini-2.5-flash")
                    resp = model.generate_content(prompt)
                    feedback_output = resp.text
                    ...
                    feedback_output = r.choices[0].message.content

                else:
                    feedback_output = "No AI key configured."

            except Exception as e:
                feedback_output = f"Feedback call failed: {e}"
